 ...
 */
class CXTSeccode {
    var $code;			//100000-999999 rand
	var $type 	= 0;		//0 english image  1 chinese image  2 Flash  3 audio
	var $width 	= 0;
	var $height 	= 0;
	var $background	= 1;
	var $adulterate	= 1;		//background shape
	var $ttf 	= 0;		//TTF font
	var $angle 	= 0;		// angle
	var $color 	= 1;
	var $size 	= 0;
	var $shadow 	= 1;
	var $animator 	= 0;		//GIF animator
	var $fontpath	= '';		// TTF path
	var $datapath	= '';		//image,flash etc. path
	var $includepath= '';		//other path
	...
	function image() {
		...
		if($this->animator == 1 && function_exists('imagegif') && function_exists('xt_load')) {
		    xt_load('library','gifmerge');
			...
			$anim = new CXTGifmerge($frame, 255, 255, 255, 0, $d, $x, $y, 'C_MEMORY');
	...
	function background() {
		...
		if(!$this->background || !$backgrounds) {
			...
			for($i = 0;$i < $this->width;$i++) {
				...
				imageline($this->im, $i, 0, $i-(isset($this->angle) ? $this->angle : 0), $this->height, $color);
